...
async def main(page: ft.Page):
    ...
    stat_grid = ft.Row(
        controls=[
            create_stat_card(
                "CPU", cpu_text, ft.Icons.MEMORY, ft.Colors.BLUE, stat_box_width
            ),
            create_stat_card(
                "GPU",
                gpu_text,
                ft.Icons.SIXTY_FPS_SELECT,
                ft.Colors.GREEN,
                stat_box_width,
            ),
            create_stat_card(
                "RAM", ram_text, ft.Icons.MEMORY, ft.Colors.ORANGE, stat_box_width
            ),
            create_stat_card(
                "Disks", disk_text, ft.Icons.FOLDER, ft.Colors.PURPLE, stat_box_width
            ),
        ],
        wrap=True,
        spacing=20,
    ...
    connection_view = ft.Column(
        ...
        controls=[
            ...
            ip := ft.TextField(
                label="Server IP",
                width=300,
                on_submit=lambda e: setattr(ip, "value", e.control.value),
            ),
        ],
        visible=True,
    )
    ...
    benchmark_view = ft.Column(
        ...
        controls=[
            app_name := ft.TextField(
                label="Game Name",
                width=300,
                on_submit=lambda e: setattr(app_name, "value", e.control.value),
            ),
            fps := ft.Text(fps),
        ],
        visible=False,
            ...
            try:
                async with websockets.connect(uri, ping_interval=None) as websocket:
                    ...
                    while True:
                        ...
                        if stats["benchmarking"]["is_running"]:
                            fps.value = stats["benchmarking"]["fps"]
                        if not app_name:
                            pass
                        if app_name:
                            print(f"Printing app, {app_name.value}")
                            await websocket.send(f"app, {app_name.value}")
                        page.update()
            except Exception as e:
                print(
                    f"Error connecting to server: {e} using {uri} - Retrying in 3 seconds..."
                )
